...
public class KetQuaXoSoMienBacService : IHostedService, IDisposable
{
    private readonly ILogger<KetQuaXoSoMienBacService> _logger;
    private Timer _timer = null!;
    private DateTime _date;
    private string _url = @"https://xoso.me/xsmb-{0}-ket-qua-xo-so-mien-bac-ngay-{0}.html";
    private IMongoCollection<KqxsMbModel> _mongoCollection;
    ...
    public KetQuaXoSoMienBacService(ILogger<KetQuaXoSoMienBacService> logger)
    {
        ...
        var sort = Builders<KqxsMbModel>.Sort.Ascending(nameof(KqxsMbModel.Id));
    ...
    private async void DoWork(object? state)
    {
        ...
        {
            var kq = await XoSoMienBac.GetData(_url, _date, _logger);
            if (kq != null)
            {
        ...
        else if (_date.Date == now.Date)
        {
            if (now.TimeOfDay > _timeStart.TimeOfDay && now.TimeOfDay < _timeStop.TimeOfDay)
            {
                ...
                if (kq != null)
                {
                    ...
                    {
                        await _mongoCollection.ReplaceOneAsync(Builders<KqxsMbModel>.Filter.Eq("NgayQuay", kq.NgayQuay), kq);
                    }
                    else
                    {

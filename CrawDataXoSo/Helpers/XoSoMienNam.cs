...
{
    public static async Task GetData(string url, string collectionName)
    {
        try
        {
            var date = new DateTime(2009, 01, 01);

            var mongo = new MongoClient(AppConstants.ConnectionStringMongoDb);
            var database = mongo.GetDatabase("Kqxs");
            database.DropCollection(collectionName);
            var collection = database.GetCollection<KqxsMnModel>(collectionName);
            ...
            while (true)
            {
                ...
                if (kq.Datas.Any())
                {
                    ...
                    Console.WriteLine($"{collectionName}: {date} - done");
                }
                else
                {
                    Console.WriteLine($"{collectionName}: {date} - fail");
                }
            ...
            Console.WriteLine($"{collectionName}: {DateTime.Now.ToString("F")} - Done");
        }
        catch (Exception e)
        {

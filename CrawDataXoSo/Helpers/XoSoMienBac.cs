using CrawDataXoSo.Configurations;
using CrawDataXoSo.Models;
...
public static class XoSoMienBac
{
    public static async Task GetData()
    {
        try
        {
            ...
            database.DropCollection("KqxsMb");
            var collection = database.GetCollection<KqxsMbModel>("KqxsMb");
            var url = "https://xoso.me/xsmb-{0}-ket-qua-xo-so-mien-bac-ngay-{0}.html";
            ...
            while (true)
            {
                ...
                kq.NgayQuay = date.ToString("d-M-yyyy");
                ...
                if (!string.IsNullOrEmpty(kq.DacBiet) && !string.IsNullOrEmpty(kq.G1))
                {
                    await collection.InsertOneAsync(kq);
                    Console.WriteLine($"KqxsMb: {date} - done");
                }
                else
                {
                    Console.WriteLine($"KqxsMb: {date} - fail");
                }
                ...
                date = date.AddDays(1);
                await Task.Delay(TimeSpan.FromMilliseconds(random.Next(60000)));
            }

            Console.WriteLine($"KqxsMb: {DateTime.Now.ToString("F")} - Done");
        }
        catch (Exception e)
        {
